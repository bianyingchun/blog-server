...
const { addComment, deleteComment, getComment, editComment, likeComment } = require('../controllers/comment');
...
const sendMailToAdminAndTargetUser = (comment) => {
    sendMail({
        ...
        html: `<p> 来自 ${comment.author.name} 的留言：${comment.content}</p><br><a href="${comment.permalink || 'javascript:;'}" target="_blank">[ 点击查看 ]</a>`
    });
};
